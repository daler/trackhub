...
#
# <!--
# The div's class is effectively a unique name across the HTML, so this is
# what we'll use for a key in the dictionaries we're creating. It will be
# stored in the id_ variable.
# -->
# <div class="bamGrayMode">
#
#   <!--
#   The span class starts with "types" and includes the track types the setting
#   is relevant to. This will be stored in the "types" variable.
#   -->
#   <span class="types bam"></span>
#
#   <!--
#   The format class contains <code> chunks. Sometimes, as shown here, there
#   are multiple options or sub-options.
#   -->
#   <div class="format">
...
def keep(tag):
    """
    Track settings have a <span class="types..."> indicating which type they're
    from. However some are examples that luckily are tagged as such (e.g.,
    bed_example).
    """
    if tag.name != 'span':
        return
...
for i in d:
    ...
    # Various filters may exclude it from being in the specs dictionary though.
    debug[_id] = div
    ...
    # Most non-hub-relevant settings are filtered out py the supported types
    # filter, but some sneak through (e.g. several only used by ENCODE).
    ...
    # We are only concerned with settings, not types. Types (bigBed, bam, etc)
    # have "type" in their format.
    ...
    # In some cases there are more than one format. In those cases, consider
    # a new _id based on the first word of the sub-option. Note that we should
    # add the divs to the debug dict as well.
    if len(formats) > 1:
...
known = {}
known.update(constants.aggregate_track_fields)
known.update(constants.assembly_fields)
known.update(constants.composite_track_fields)
known.update(constants.track_fields)
for i in constants.track_typespecific_fields.values():
    known.update(i)
known.update(constants.view_track_fields)

already_known = set(known.keys()).intersection(specs.keys())
not_in_library = set(known.keys()).difference(specs.keys())
not_known = set(specs.keys()).difference(known.keys())
